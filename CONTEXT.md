...
**Anonymized dossier**:
A shareable, PII-free presentation of a Candidate for pharmacy clients — six fixed sections (accroche, métier & expérience, compétences & logiciels, mobilité, disponibilité & contrat, points forts) generated by AI, editable by the recruiter with autosave on the Candidate Documents tab, then exportable as PDF. Empty sections are omitted from the PDF and preview. Full AI regenerate overwrites all sections (with confirm); the recruiter may edit again afterward. The same PII guard applies on AI generation and on manual save (refuse persist if email, phone, or forbidden identity tokens appear). Distinct from `cvSummary` (internal recruiter notes) and from presentation emails to pharmacies.
