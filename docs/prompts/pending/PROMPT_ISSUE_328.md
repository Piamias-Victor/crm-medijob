...
1. **Explique en français simple** : **Accepter** le Devis courant enregistre le **CA** (montant HT **une fois**, date = acceptation). Pastille **état commercial** à côté du statut Mission (Sans devis → Envoyé → Accepté → Facturé) — on **n’ajoute pas** ça dans `MissionStatus`. Recruteur voit l’état, pas CA/Marge. Direction tape la **Marge** et coche **Facturé** (date). Mission **Annulée** → CA 0. Communication peut accepter (`crm.write`).
...
- Marge saisie Direction ; marge moteur indicative
...
- [ ] Recruteur/Communication : pas CA/Marge ; peuvent accepter
...
- `finance.view` pour CA/Marge/Facturé
...
- [ ] Recruteur accepte → pastille Accepté ; pas de Marge à l’écran
- [ ] Direction : CA = HT, tape Marge, coche Facturé → mois CA inchangé
- [ ] Annuler la Mission → CA disparaît du suivi (0)
- [ ] Communication : peut accepter, ne voit pas la Marge
