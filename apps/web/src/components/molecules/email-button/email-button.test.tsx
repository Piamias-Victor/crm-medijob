...
describe('EmailButton', () => {
  ...
  it('shows a toast when Gmail popups are blocked', () => {
    vi.mocked(openEmailCompose).mockReturnValue(false)
    render(<EmailButton to="candidat@example.com" />)
    fireEvent.click(screen.getByRole('button', { name: /envoyer un email/i }))
    expect(useToastStore.getState().toasts.at(-1)?.message).toBe(COMPOSE_POPUP_BLOCKED)
  })
