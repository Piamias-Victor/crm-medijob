...
export type AppliedPending = {
  recipientId: string
  mission: { badakanId: string; enterpriseId: string | null }
}
...
export async function attachPharmacyApplyRecipients(
  db: PrismaClient,
  pending: AppliedPending[],
): Promise<PharmacyApplyEmailDueRow[]> {
  const enterpriseIds = [
    ...new Set(pending.flatMap((row) => (row.mission.enterpriseId ? [row.mission.enterpriseId] : []))),
  ]
